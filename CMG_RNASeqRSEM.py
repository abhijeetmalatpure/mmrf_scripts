...
log_filename = os.path.join(f"log/rsemNantomics_{str(int(epoch_time))}.log")
    ...
    with gzip.open(vcf_filename, mode='rt') as vcf:
        ...
        for line in vcf:
            ...
            if line.startswith('##SAMPLE=<ID=NORMAL,'):
                # print(line)
                normal_id = \
                dict(item.split('=') for item in line[line.find('<') + 1:line.find('>')].replace("\"", '').split(','))[
                    'File'].split('.')[0]
...
def vcf2maf_call(sample_id, filename, vcftype, normal_id):
    ...
    vcffile = join(rsem_root, filename)
